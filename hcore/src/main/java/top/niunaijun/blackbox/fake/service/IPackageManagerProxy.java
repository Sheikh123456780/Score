...
public class IPackageManagerProxy extends BinderInvocationStub {
    ...
    public IPackageManagerProxy() {
        super(resolvePackageManagerBinder());
    }
    ...
    private static Object resolvePackageManagerBinder() {
        ...
        try {
            ...
            Slog.w(TAG, "resolvePackageManagerBinder: sPackageManager failed: " + t.getMessage());
        }

        // 2) Fall back to ServiceManager.getService("package")
        try {
        ...
        // 3) Last resort — dummy binder so super() doesn't NPE
        Slog.e(TAG, "resolvePackageManagerBinder: all resolution paths failed, using dummy binder");
        return new android.os.Binder();
    }

    @Override
    protected Object getWho() {
        // Try ActivityThread first
        try {
            Object pm = BRActivityThread.get().sPackageManager();
            if (pm != null) {
                Slog.d(TAG, "getWho: returning sPackageManager");
                return pm;
            }
        } catch (Throwable t) {
            Slog.w(TAG, "getWho: sPackageManager failed: " + t.getMessage());
        }
        ...
        // Return null → ClassInvocationStub will skip the hook safely
        Slog.e(TAG, "getWho: PM service unavailable, hook will be skipped");
        return null;
    }
    ...
    protected void inject(Object baseInvocation, Object proxyInvocation) {
        try {
            ...
            Slog.w(TAG, "inject: _set_sPackageManager failed: " + t.getMessage());
        }

        try {
            replaceSystemService("package");
        } catch (Throwable t) {
            Slog.w(TAG, "inject: replaceSystemService failed: " + t.getMessage());
        }
        ...
        try {
            ...
            if (packageManager != null) {
                try {
                    ...
                    Slog.w(TAG, "inject: disableApplicationInfoCache failed: " + t.getMessage());
                }
                try {
                    ...
                    Slog.w(TAG, "inject: ApplicationPackageManager.mPM set failed: " + e.getMessage());
                }
            }
        } catch (Throwable t) {
            Slog.w(TAG, "inject: system context path failed: " + t.getMessage());
        }
    }
