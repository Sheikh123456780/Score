package com.Score.app.dispatcher;
...
import com.Score.entity.ServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = ScoreCore.get().getHandler();
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            ScoreCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = ScoreCore.getBActivityManager().onServiceUnbind(proxyIntent, BActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    ScoreCore.getBActivityManager().onServiceDestroy(proxyIntent, BActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    ScoreCore.getBActivityManager().onServiceDestroy(intent, BActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
