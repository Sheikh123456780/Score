...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private static String sanitizeWebViewDataDirectorySuffix(String suffix) {
        if (TextUtils.isEmpty(suffix)) {
            return "default_wv";
        }
        String sanitized = suffix.replaceAll("[^a-zA-Z0-9._-]", "_");
        return TextUtils.isEmpty(sanitized) ? "default_wv" : sanitized;
    }

    public static void configureWebViewDataDirectory(String packageName, String processName) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            if (TextUtils.isEmpty(packageName) || TextUtils.equals(packageName, ScoreCore.getHostPkg())) {
                return;
            }
            ...
            // Keep the profile stable across process restarts. A PID-based suffix
            // creates a fresh Chromium cache on every launch and causes POST/OAuth
            // restore to fail with net::ERR_CACHE_MISS.
            String rawSuffix = "wv_u" + getUserId() + "_" + packageName + "_" + processName;
            String sanitizedSuffix = sanitizeWebViewDataDirectorySuffix(rawSuffix);

            try {
                WebView.setDataDirectorySuffix(sanitizedSuffix);
            } catch (IllegalStateException e) {
                Slog.w(TAG, "WebView data directory suffix already set: " + sanitizedSuffix);
            } catch (Throwable t) {
                Slog.e(TAG, "Failed to set WebView data directory suffix: " + sanitizedSuffix, t);
            }
        }
    }
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        // Safe Multi-Process WebView Data Directory Isolation
        ...
        try {
            ...
            if (application == null) {
                Log.e(TAG, "makeApplication error!");
