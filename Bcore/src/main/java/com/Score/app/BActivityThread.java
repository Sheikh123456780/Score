...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            // WebView permits only one process per data directory. Virtual
            // processes can share the same package/process name, so include
            // the real Linux PID and sanitize the suffix before first use.
            String webViewSuffix = getUserId() + "_" + packageName + "_"
                    + processName + "_pid" + android.os.Process.myPid();
            WebView.setDataDirectorySuffix(
                    webViewSuffix.replaceAll("[^A-Za-z0-9_.-]", "_"));
        }
