...
import com.Score.utils.TrieTree;
import java.io.File;
  ...
  public void addRedirect(String origPath, String redirectPath) {
    ...
    public String redirectPath(String path) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            String target = mRedirectMap.get(key);
            if (TextUtils.isEmpty(target)) break;
            String redirected = path.replace(key, target);
            if (TextUtils.equals(path, redirected)) break;
            path = redirected;
        }
        return path;
  }
  ...
  public void enableRedirect(Context context) {
    ...
    try {
      ApplicationInfo packageInfo =
          ScoreCore.getBPackageManager()
              .getApplicationInfo(
                  packageName, PackageManager.GET_META_DATA, BActivityThread.getUserId());
      int systemUserId = ScoreCore.getHostUserId();
      ...
      if (ScoreCore.getContext().getExternalCacheDir() != null
        ...
        if (sdcardAndroidFile.exists()) {
          ...
          if (childDirs != null) {
            ...
            rule.put("/sdcard/Android", BEnvironment.getExternalAndroidRootDir(
                BActivityThread.getUserId()).getAbsolutePath());
            rule.put(androidDir, BEnvironment.getExternalAndroidRootDir(
                BActivityThread.getUserId()).getAbsolutePath());
          }
        } else {
          rule.put("/sdcard/Android", BEnvironment.getExternalAndroidRootDir(
