...
import com.Score.core.system.BProcessManagerService;
import com.Score.core.system.ISystemService;
import com.Score.core.system.ProcessRecord;
import com.Score.core.system.user.BUserHandle;
import com.Score.core.system.user.BUserInfo;
import com.Score.core.system.user.BUserManagerService;
...
import com.Score.utils.AbiUtils;
import com.Score.utils.FileUtils;
import com.Score.utils.Slog;
import com.Score.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ScoreCore.getHostPkg())) {
            try {
                return ScoreCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ScoreCore.getHostPkg())) {
            try {
                return ScoreCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = ScoreCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = ScoreCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(ScoreCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (ScoreCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = ScoreCore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }
    ...
    /**
     * Auto-grant audio/microphone permissions to newly installed apps
     */
    ...
    private int updateFlags(int flags, int userId) {
        if ((flags & (PackageManager.MATCH_DIRECT_BOOT_UNAWARE
            ...
            // Caller expressed explicit opinion
        } else {
