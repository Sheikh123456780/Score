package com.Score.core.system.pm;
...
import com.Score.utils.ArrayUtils;
...
//                    }
        ...
        try {
            base = ScoreCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ActivityInfo generateActivityInfo(BPackage.Activity a, int flags, BPackageUserState state, int userId) {
        ...
        // Force-declare that this activity handles orientation/screen-size/
        // keyboard config changes itself, regardless of what the reflection-
        // based PackageParser for this API level actually captured from the
        // hosted app's own <activity android:configChanges="..."> attribute.
        //
        // Without this, the real system (ActivityThread/WindowManager, which
        // still runs the hosted Activity for real inside our process) treats
        // configChanges as 0 for any activity whose declared value didn't
        // survive our manifest parsing, and destroys+recreates the Activity on
        // every rotation. For activities holding a WebView mid-navigation
        // (e.g. an OAuth/login WebView that got there via a POST, such as a
        // Twitter/X login flow), Android's WebView state restore then tries to
        // reload that POST navigation from the disk cache and fails with
        // net::ERR_CACHE_MISS ("Page not available") instead of just resizing
        // in place. Declaring these bits pre-emptively makes every hosted
        // activity rotation-safe.
        ai.configChanges |= ActivityInfo.CONFIG_ORIENTATION
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = ScoreCore.getPackageManager().getApplicationInfo(ScoreCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = ScoreCore.getPackageManager()
...
//            ApplicationInfo base = ScoreCore.getContext().getApplicationInfo();
