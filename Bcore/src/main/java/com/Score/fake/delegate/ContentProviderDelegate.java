...
import com.Score.fake.service.context.providers.SystemProviderStub;
...
public class ContentProviderDelegate {
    ...
    public static void update(Object holder, String auth) {
        ...
        switch (auth) {
            ...
            case "telephony":
            case "settings":
                bContentProvider = new SystemProviderStub().wrapper(iInterface, ScoreCore.getHostPkg());
                break;
            default:
                bContentProvider = new ContentProviderStub().wrapper(iInterface, ScoreCore.getHostPkg());
                break;
        }
        if (BuildCompat.isOreo()) {
    ...
    public static void init() {
        ...
        ScoreCore.getContext().getContentResolver().call(Uri.parse("content://settings"), "", null, null);
        Object activityThread = ScoreCore.mainThread();
        ...
        for (Object value : map.values()) {
            ...
            if (!sInjected.contains(providerName)) {
                ...
                BRActivityThreadProviderClientRecordP.get(value)._set_mProvider(new ContentProviderStub().wrapper(iInterface, ScoreCore.getHostPkg()));
