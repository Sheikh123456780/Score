...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public List<ResolveInfo> queryIntentActivities(Intent intent, int flags, String resolvedType, int userId) {
        try {
            // Prefer the virtual package manager. Facebook must resolve to
            // the cloned instance, never to the host/physical application.
            if (internalResult == null || internalResult.isEmpty()) {
                String pkg = intent.getPackage();
                if (pkg == null && intent.getComponent() != null) {
                    pkg = intent.getComponent().getPackageName();
                }
                if ("com.facebook.katana".equals(pkg)
                        || "com.facebook.orca".equals(pkg)
                        || "com.facebook.lite".equals(pkg)) {
                    Slog.w(TAG, "No virtual launcher found for Facebook package: " + pkg);
                    return Collections.emptyList();
                }

                // Preserve the existing host fallback for other external intents.
